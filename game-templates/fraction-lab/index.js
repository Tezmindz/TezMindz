 ...
 * 100% Data-Driven Visual Fraction Game
 */
...
export class FractionLabGame {
  ...
  render() {
    ...
    header.innerHTML = `
      <span class="tm-badge">🍕 Fractions & Ratios</span>
      <h2 class="tm-prompt">${this.escapeHtml(this.data.prompt || 'Find the matching fraction')}</h2>
    `;
    root.appendChild(header);

    // Visual circle
    const visual = new FractionCircle(this.data.numerator, this.data.denominator, this.config.sliceColor || '#ec4899');
    visual.render(root);
