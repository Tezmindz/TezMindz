/**
 * Number Kingdom Game Engine
 * World 2: Number Sense, Place Value Systems, Divisibility & Clue Riddles
 * TezMindz 5-Stage Mastery Architecture
 */
import { PlaceValueBoard } from './components/PlaceValueBoard.js';
import { DivisibilityScanner } from './components/DivisibilityScanner.js';

export class NumberKingdomGame {
  constructor(container, data, config = {}) {
    ...
    if (!this.container) {
      throw new Error('[NumberKingdomGame] Target container not found.');
    }
  ...
  render() {
    ...
    root.className = 'tm-game-number-kingdom';
    ...
    header.innerHTML = `
      <span class="tm-badge">👑 Number Kingdom: Place Value Quest</span>
      <h2 class="tm-prompt">${this.escapeHtml(this.data.prompt || 'Solve the place value mystery')}</h2>
    ...
    if (this.currentStage === 'STAGE_A') {
      ...
      const qList = (this.data.discoveryGuidingQuestions || [
        "How does a digit's value change when shifted from Tens to Hundreds?",
        "What is the difference between face value and place value?",
        "Why does adding 1 to 999 trigger a jump into the Thousands column?"
      ]).map(q => `<li>${this.escapeHtml(q)}</li>`).join('');

      discovery.innerHTML = `
        <div class="tm-discovery-title">🔍 Explore & Observe:</div>
        <ul class="tm-discovery-list">${qList}</ul>
        <div class="tm-discovery-sub">Inspect the place-value column weights below.</div>
      `;
      root.appendChild(discovery);
    }

    // 3. Interactive Playground (Hidden in Stage E if anti-memorisation)
    const hideBlocks = this.currentStage === 'STAGE_E' && this.data.antiMemorisation?.hideBlocks;
    if (!hideBlocks) {
      ...
      abstractNotice.innerHTML = `
        ...
        <p>Solve this royal number riddle using pure arithmetic logic without place-value visual blocks.</p>
      `;
    ...
    if (this.currentStage === 'STAGE_C' && this.data.ruleDiscovery) {
      ...
      ruleBox.innerHTML = `
        <div class="tm-rule-prompt">${this.escapeHtml(this.data.ruleDiscovery.statementPrompt || 'State the discovered place value rule:')}</div>
    ...
    feedback.id = 'tm-number-feedback';
  ...
  handleOptionSelect(selectedText, buttonElement) {
    ...
    const fb = this.container.querySelector('#tm-number-feedback');
    if (fb) {
      fb.style.display = 'block';
      fb.className = `tm-feedback-panel ${isCorrect ? 'tm-feedback-correct' : 'tm-feedback-wrong'}`;
      fb.innerHTML = `
        <div class="tm-feedback-title">${isCorrect ? '🎉 Royal Decryption Master!' : '💡 Royal Correction'}</div>
        <div class="tm-feedback-text">
          ${isCorrect ? 'Your place-value deduction is 100% accurate!' : `Correct Number: <strong>${this.escapeHtml(this.data.correctAnswer)}</strong>`}
        </div>
...
export function initGame(container, data, config) {
  return new NumberKingdomGame(container, data, config);
}
